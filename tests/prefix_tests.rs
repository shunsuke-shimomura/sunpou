...
fn base_prefix_unchanged() {
    ...
    let s = Scalar::<Length>::from_raw(1000.0);
...
#[test]
fn kilo_prefix() {
    let s = Scalar::<Length, Kilo>::from_raw(7.0); // 7 km
...
#[test]
fn nano_prefix() {
    let s = Scalar::<Length, Nano>::from_raw(500.0); // 500 nm
...
#[test]
fn mega_prefix() {
    let s = Scalar::<Mass, Mega>::from_raw(2.5); // 2.5 × 10^6 kg
...
fn rescale_kilo_to_base() {
    let km = Scalar::<Length, Kilo>::from_raw(7.0);
...
fn rescale_nano_to_micro() {
    let nm = Scalar::<Length, Nano>::from_raw(500.0); // 500 nm
...
fn rescale_base_to_kilo() {
    let m = Scalar::<Length, Base>::from_raw(7000.0);
...
fn rescale_roundtrip() {
    let original = Scalar::<Length, Kilo>::from_raw(42.0);
...
#[test]
fn add_same_prefix() {
    let a = Scalar::<Length, Kilo>::from_raw(3.0); // 3 km
    let b = Scalar::<Length, Kilo>::from_raw(4.0); // 4 km
    let c = a + b;
...
#[test]
fn sub_same_prefix() {
    let a = Scalar::<Length, Nano>::from_raw(500.0);
    let b = Scalar::<Length, Nano>::from_raw(200.0);
...
fn mul_kilo_times_kilo() {
    ...
    let a = Scalar::<Length, Kilo>::from_raw(3.0);
    let b = Scalar::<Length, Kilo>::from_raw(4.0);
...
fn mul_nano_times_mega() {
    ...
    let a = Scalar::<Length, Nano>::from_raw(5.0);
    let b = Scalar::<InvTime, Mega>::from_raw(3.0);
...
fn mul_base_times_kilo() {
    ...
    let mass = Scalar::<Mass>::from_raw(10.0);
    let vel = Scalar::<Velocity, Kilo>::from_raw(5.0);
...
fn div_kilo_by_base() {
    ...
    let dist = Scalar::<Length, Kilo>::from_raw(100.0);
    let time = Scalar::<Time>::from_raw(10.0);
...
fn div_mega_by_kilo() {
    ...
    let energy = Scalar::<Energy, Mega>::from_raw(12.0);
    let force = Scalar::<Force, Kilo>::from_raw(4.0);
...
fn f64_mul_preserves_prefix() {
    let s = Scalar::<Length, Kilo>::from_raw(3.0);
    let doubled = s * 2.0;
...
fn precision_nano_times_giga() {
    ...
    let a = Scalar::<Length, Nano>::from_raw(5.0);
    let b = Scalar::<InvTime, Giga>::from_raw(3.0);
...
fn precision_addition_same_scale() {
    ...
    let a = Scalar::<Length, Nano>::from_raw(1.5);
    let b = Scalar::<Length, Nano>::from_raw(2.5);
    let c = a + b;
...
fn uom_cross_validation_with_prefix() {
    ...
    let our_dist = Scalar::<Length, Kilo>::from_raw(100.0); // 100 km
    let our_time = Scalar::<Time>::from_raw(10.0);          // 10 s
...
fn add_assign_with_prefix() {
    let mut a = Scalar::<Length, Kilo>::from_raw(3.0);
    a += Scalar::<Length, Kilo>::from_raw(4.0);
